...
#define NEWTON_ITER 11 // Experimentally min value for high enough precision
#define PRIME_MAX 312  // We need primes up to including 311
#define H_SIZE 8
#define K_SIZE 64
...
static uint32_t extract_fraction(double x)
{
    return (uint64_t)(x * (1LL << 32));
}

void init(uint32_t *h, uint32_t *k)
{
    size_t prime_idx = 0;
    uint8_t is_prime[PRIME_MAX - 2];
    for (size_t prime = 0; prime < (PRIME_MAX - 2); prime++)
    {
        is_prime[prime] = 1;
    }
    for (size_t prime = 0; prime < (PRIME_MAX - 2); prime++)
    {
        ...
        if (prime_idx < H_SIZE)
        {
            double hprime = prime + 2;
            hprime = sqrt_newton(hprime);

            h[prime_idx] = extract_fraction(hprime);
        }
        if (prime_idx < K_SIZE)
        {
            double kprime = prime + 2;
            kprime = cbrt_newton(kprime);
            k[prime_idx] = extract_fraction(kprime);
        }
        prime_idx++;
        for (size_t n = p+2; n < (PRIME_MAX - 2); n++)
        {
            size_t val = (prime + 2) * n - 2;
            if (val + 1 > (PRIME_MAX - 2))
            {
                break;
            }
            is_prime[val] = 0;
